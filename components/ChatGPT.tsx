...
export default function ChatGPT({ isOpen, onClose }: ChatGPTProps) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (SpeechRecognition) {
        ...
        recognitionInstance.onresult = (event: any) => {
          ...
          if (interimTranscript) {
            console.log('🔄 Пр��межуточный текст:', interimTranscript)
          }
          ...
          if (finalTranscript) {
            ...
            console.log('⏱️ Запускаем тай��ер автоотправки (2 сек)')
        ...
        recognitionInstance.onspeechstart = () => {
          ...
          if (silenceTimer) {
            ...
            console.log('🔇 Таймер тиши��ы очищен (речь возобновилась)')
          }
        }
      ...
      if (window.speechSynthesis) {
        ...
        // Инициализируем г��лоса (неко��орчие браузеры загружают их асинхронно)
        const loadVoices = () => {
  ...
  const generateJarvisResponse = async (userMessage: string, conversationHistory: Message[]): Promise<string> => {
    try {
      ...
      return 'Произошла ошибка при генерации отве��а. Проверьте подключение к интернету и попробуйте снова. 🌐'
    }
  }
