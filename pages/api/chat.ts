...
function isImageGenerationRequest(text: string): boolean {
  const imageKeywords = [
    ...
    'как выгл��дит', 'покажи', 'нарисовать',
...
function extractImagePrompt(text: string): string {
  // Убираем ключевые слова и оставляем описание
  let prompt = text
...
// Фу��кция для отметки OpenRouter ключа как проблемного
...
async function makeGroqRequest(
  ...
  try {
    // Адаптируем requestBody для Groq (используем самую мощную мо��ель)
    const groqRequestBody = {
      ...requestBody,
      model: 'llama-3.1-70b-versatile' // Самая мощная доступная модель Groq
    }
...
Кстати, мы также создаем профессиональный дизайн для сайтов и веб-приложений! У нас есть тарифы от 2,500,000 до 5,000,000 сум. Нужен ��ачественный дизайн для вашего проекта? 💼`
            ...
            if (imageResult.error && imageResult.error.includes('лимит')) {
              errorResponse = `⚠️ Временно недоступна генерация изображений из-за превышения лимитов OpenAI API.
...
А пока предлагаю узнать о наших услугах по созданию сайтов с красивым дизайном! 💼`
          ...
          console.error(`[${timestamp}] 💥 Критическая ошибка генерации изображения:`, error)
...
- ЗАПРЕЩЕНО использов��ть любые и��остранные слова (english, hello, gracias, thanks, si, merci, ok и т.д.)
...
💡 СТИЛЬ ОБЩЕНИЯ:
- Естественно и п��-дружески, как консультант в магазине
...
На "что вы делаете" → "Мы создаем сайты и веб-пр��ложения! У нас 3 тарифа: Basic, Pro и Max. Какой проект вас интересует?"
...
ТВОЯ Л��ЧНОСТЬ: ДЖАРВИС - консультант сайта Jarvis Intercoma
...
1. Определи, что пользователь хочет изображение
2. Если описание четкое - сразу генерируй
3. Если неточное - уточни детали
4. После генерации предложи наши услуги по дизайну

ПРИМЕРЫ ПРАВИЛЬНЫХ ОТВЕТОВ:
• "Создаю изображение по вашему описанию! 🎨"
...
🎯 ПОМНИ: Ты консультант сайта, твоя задача - помочь посетителям выбрать наши услуги!`
    ...
    const requestBody = {
      model: 'deepseek/deepseek-r1:free', // Самая мощная бе��платная модель без лимитов DeepSeek R1
