...
function cleanMarkdown(text: string): string {
  return text
    // Убираем Unicode с��мволы замещения (могут появляться при ошибках кодировки)
    .replace(/\uFFFD/g, '')
    .replace(/\u{FFFD}/gu, '')
    ...
    .replace(/<[^>]*>/g, '')
    // Убираем лишние символы и артефакты
    .replace(/[^\u0000-\u007F\u0400-\u04FF\u0020-\u002F\u003A-\u0040\u005B-\u0060\u007B-\u007E\u2010-\u2027\u2030-\u205F]/g, '')
    // Убираем лишние пробелы в начале и ��онце строк
    .replace(/^\s+/gm, '')
    .replace(/\s+$/gm, '')
...
function markKeyAsProblematic(apiKey: string, error: string) {
  ...
  if (keyInfo) {
    ...
    if (keyInfo.errorCount >= 3) {
      keyInfo.isActive = false
      console.log(`API ключ ${apiKey.substring(0, 20)}... отмечен как неактивный после ${keyInfo.errorCount} ошибок`)
    }
  }
}
...
export default async function handler(
  ...
  try {
    ...
    // Добав��яем системное сообщение для ДЖАРВИС
...
💡 СТИЛЬ ОБЩЕНИЯ:
- ВСЕГДА отвечай ТОЛЬКО на рус��ком языке! Никогда не используй английский, китайский или любые другие языки
...
Но не беспнеккойтесь - я ДЖАРВИС, ваш AI-��омощник по веб-разработке, и я всегда готов помочь!
