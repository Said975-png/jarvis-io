...
function isImageGenerationRequest(text: string): boolean {
  const imageKeywords = [
    ...
    'как выглядит', 'покажи', 'нарисовать',
...
function getNextAvailableOpenRouterKey(excludeKey?: string): string | null {
  // Сначала п��обуе�� активные ключи, исключая переданный
...
- Твоя цель - помочь посетителю и показать наш�� услуги
...
4. После генерации предл��жи наши ��слуги по дизайну
...
🎯 ПОМНИ: Ты консультант сайта, твоя задача - помочь посетителям выбрать наши услуги!`
    ...
    if (groqResult.success) {
      console.log(`[${timestamp}] ✅ Groq успе��но ответил`)
