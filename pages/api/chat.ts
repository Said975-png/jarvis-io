...
function extractImagePrompt(text: string): string {
  ...
  let prompt = text
    ...
    .replace(/сгенерируй к��ртинку/gi, '')
    .replace(/нарисуй/gi, '')
...
function checkAndUpdateLimit(ip: string): { allowed: boolean; remaining: number } {
  ...
  // Если пользовател�� не найден или время сброса пр��шло
...
// Систем�� мно��ественных API ключей
...
function getNextAvailableOpenRouterKey(excludeKey?: string): string | null {
  // Сначала пробуе�� активные ключи, исключая переданн��й
  ...
  // ��сли активных ключей нет, пробуем сбросить счетчики ошибок
...
📋 BASIC (2,500,000 сум):
• Прост��е сайты и лендинги
• Красивый дизайн
• Мобильная версия
• Базовый функционал
• Идеально для: визиток, портфол��о, небольшого бизнеса
