...
function checkAndUpdateLimit(ip: string): { allowed: boolean; remaining: number } {
  ...
  // Периоди��еская очистка старых записей (каждые 100 запросов)
  if (Math.random() < 0.01) {
...
На "сколько стоит" → "У нас 3 тарифа: Basic (2,500,000 сум), Pro (4,000,000 сум), Max (5,000,000 сум). Рас��кажите о вашем проекте, подберу оптимальный!"
...
🎯 ПОМНИ: Ты консультант сайта, твоя задача - помочь посетителям выбрать наши услуги!`
    ...
    console.log(`[${timestamp}] === НАЧИНАЕМ К��СКАДНЫЙ ЗАПРОС ===`)
