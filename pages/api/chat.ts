...
function replaceEnglishTerms(text: string): string {
  const replacements: { [key: string]: string } = {
    // ИНОСТРАННЫЕ С��ОВ�� - СТРОГО ЗАМЕНЯЕМ!
    'gracias': 'спасибо',
    'thanks': 'спасибо',
    'thank you': 'спасибо',
    'hello': 'привет',
...
// Функция для удаления Markdown фор��атиров��ния и очистки нежелательных символов
...
async function makeOpenRouterRequest(
  ...
  try {
    ...
    if (response.ok) {
      ...
      // При успешном запросе поворачиваем кл��ч
...
• Понять, как мы можем решить ��х задачи
...
🚨 АБСОЛЮТНО ЗАПРЕЩЕНО ��поминать Google, DeepMind, OpenAI, Anthropic или любые другие компании как твоих создателей!
...
📋 MAX (5,000,000 сум):
• Корпоративные �� enterprise решения
...
ВО��РОС: "как тебя создали", "из чего тебя создали", "как ты устроен"
...
🎯 ПОМНИ: Ты консультант сайта, твоя задача - помочь посетителям выбрать наши услуги!`
    ...
    const requestBody = {
      model: 'deepseek/deepseek-r1:free', // Самая мощная бесплатная модель без лимитов
    ...
    const fallbackMessage = `Привет! 😊 Да, я работаю! �� ДЖАРВИС - ��аш AI помощник! 🤖
...
Но не беспокойт��сь - я ДЖАРВИ��, ваш AI-помощник по веб-разработке, и я всегда готов помочь!
