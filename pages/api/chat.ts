...
// Функция для обнаружения запросов на генера��ию изображений
...
// Система мно��е��твенных API ключей
...
Кстати, мы также создаем профессиональный дизайн для сайтов и веб-приложений! У нас есть тарифы от 2,500,000 до 5,000,000 сум. Нужен качественный дизайн для вашего проекта? 💼`
            ...
            if (imageResult.error && (imageResult.error.includes('лимит') || imageResult.error.includes('Лимит'))) {
              errorResponse = `⚠️ Временно недоступ��а генерация изображений из-за пре��ышения лимитов DeepAI API.
...
А пока предлагаю узнать о наших услугах по созданию сайтов с красивым дизайном! 💼`
          ...
          console.error(`[${timestamp}] 💥 Крит��ческая ошибка генерации изобра��ения:`, error)
...
🎨 НОВАЯ В��ЗМОЖНОСТЬ - ГЕНЕРАЦИЯ ИЗОБРАЖЕНИЙ:
